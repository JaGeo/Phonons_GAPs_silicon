...
for ipot, pot in enumerate(potential_filenames[0:], 0):
    ...
    foldername = "Results"
    mpids=["mp-1072544","mp-1079297","mp-1095269","mp-1200830","mp-1203790","mp-149","mp-16220","mp-165","mp-168","mp-571520", "mp-971661","mp-971662","mp-999200"]
    # for istructure  in [1, 2, 3, 4, 5, 6, 8, 9, 10, 12, 13, 15, 18, 21, 22, 23, 24, 25, 26, 27, 28, 31, 32, 33, 34]:
    #21 has very small imaginary modes and will therefore not be included in our list
    for iistructure,istructure in enumerate([5, 6, 9, 15, 18,  22, 23, 24, 25, 28, 32, 33, 34]):
        ...
        for DISPLACEMENT_DISTANCE in DISTANCES:
            ...
            with open("Results/results_" + str(DISPLACEMENT_DISTANCE) + ".txt", 'a') as f:
                f.write(str(potential_names[ipot]) + ' '+str(istructure)+ ' ' + str(mpids[iistructure]) + ' ' + str(rms) + ' ' + str(
