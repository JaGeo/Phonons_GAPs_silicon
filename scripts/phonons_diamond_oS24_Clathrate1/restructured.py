...
potential_filenames = [str(os.path.join(path_to_potential,"gp_iter6_sparse9k.xml")),
 ...
 str(os.path.join(path_to_potential,"07_fit/03_fas_series/scratch_wd_nsp9k_0.0001/gp_iter6C.xml")),
 str(os.path.join(path_to_potential,"07_fit/03_fas_series/scratch_wd_nsp9k_0.0010/gp_iter6C.xml")),
 str(os.path.join(path_to_potential,"07_fit/03_fas_series/scratch_wd_nsp9k_0.0100/gp_iter6C.xml")),
 str(os.path.join(path_to_potential,"07_fit/03_fas_series/scratch_wd_nsp9k_0.1000/gp_iter6C.xml")),
...
for DISPLACEMENT_DISTANCE in DISTANCES:
    with open("Results/results_" + str(DISPLACEMENT_DISTANCE) + ".txt", 'w') as f:
        f.write("Pot Structure RMS2 Mean_absolute_perc_error\n")

#with open("Results/results_thermalprop.txt", 'w') as f:
#    f.write("Pot Structure RMS_zpe RMS_free_energy \n")
    ...
    for istructure, structure in enumerate(structures):
        for DISPLACEMENT_DISTANCE in DISTANCES:
            ...
            runner_castep = PhonopyFiniteDisplacements(
                filename=os.path.join("../DFT_Benchmark_CASTEP", str(castep_calc_ids[istructure]) + '_phonopy.yaml'), kpoint_density=kpoint_density)
            ...
            #comparezpe = CompareZPE(runner, runner_castep)



            #rms_zpe = comparezpe.get_rms_zpe()
            #rms_free_energy = comparezpe.get_rms_free_energy(0, 1000, 10)

            #with open("Results/results_thermalprop.txt", 'a') as f:
            #    f.write(str(potential_names[ipot]) + ' ' + str(structure_names[istructure]) + ' ' + str(
            #        rms_zpe) + ' ' + str(rms_free_energy) + '\n')
